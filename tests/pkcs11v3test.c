                               ...
                               CK_OBJECT_HANDLE privKey)
{
    ...
    ret = funcListExt->C_EncapsulateKey(session, &mech, pubKey, secretTmpl,
                                        secretTmplCnt, &encapKey, NULL, &ctLen);
    if (ret == CKR_BUFFER_TOO_SMALL)
        ret = CKR_OK;
    else {
        CHECK_CKR_FAIL(ret, CKR_BUFFER_TOO_SMALL,
                       "ML-KEM Encapsulate size query");
    }
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_DecapsulateKey(session, &mech, privKey, ciphertext,
                                            ctLen, secretTmpl, secretTmplCnt,
                                            &decapKey);
...
static CK_RV test_mlkem_encap_decap_fail(void* args)
{
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_DecapsulateKey(session, &mech, privB, ciphertext,
                                            ctLen, secretTmpl, secretTmplCnt,
                                            &decapKey);
...
static CK_RV test_mlkem_bad_mech_params(void* args)
{
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_EncapsulateKey(session, &mech, pub, secretTmpl,
                                            secretTmplCnt, &secret, NULL,
                                            &ctLen);
        CHECK_CKR_FAIL(ret, CKR_MECHANISM_PARAM_INVALID,
                       "ML-KEM Encapsulate bad mechanism parameter");
    }
    if (ret == CKR_OK) {
        ret = funcListExt->C_DecapsulateKey(session, &mech, priv, dummyCt,
                                            sizeof(dummyCt), secretTmpl,
                                            secretTmplCnt, &secret);
        CHECK_CKR_FAIL(ret, CKR_MECHANISM_PARAM_INVALID,
...
static CK_RV test_mlkem_key_validation(void* args)
{
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_DecapsulateKey(session, &mech, pub, dummyCt,
                                            sizeof(dummyCt), secretTmpl,
                                            secretTmplCnt, &secret);
        CHECK_CKR_FAIL(ret, CKR_KEY_HANDLE_INVALID,
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_EncapsulateKey(session, &mech, mldsaPub, secretTmpl,
                                            secretTmplCnt, &secret, NULL,
                                            &ctLen);
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_DecapsulateKey(session, &mech, mldsaPriv, dummyCt,
                                            sizeof(dummyCt), secretTmpl,
                                            secretTmplCnt, &secret);
        CHECK_CKR_FAIL(ret, CKR_KEY_TYPE_INCONSISTENT,
...
static CK_RV test_mlkem_initial_states(void* args)
{
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_EncapsulateKey(session, &mech, pub, secretTmpl,
                                            secretTmplCnt, &encapKey, NULL,
                                            &ctLen);
        if (ret == CKR_BUFFER_TOO_SMALL)
            ret = CKR_OK;
        else {
            CHECK_CKR_FAIL(ret, CKR_BUFFER_TOO_SMALL,
                           "ML-KEM Encapsulate size query (states)");
        }
    }
    if (ret == CKR_OK) {
        ciphertext = (CK_BYTE*)malloc(ctLen);
    ...
    if (ret == CKR_OK) {
        ret = funcListExt->C_DecapsulateKey(session, &mech, priv, ciphertext,
                                            ctLen, secretTmpl, secretTmplCnt,
                                            &decapKey);
