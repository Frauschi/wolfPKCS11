        ...
        case CKM_ML_KEM_KEY_PAIR_GEN:
            ...
            *phPublicKey = *phPrivateKey = CK_INVALID_HANDLE;
            ...
            rv = NewObject(session, CKK_ML_KEM, CKO_PUBLIC_KEY,
                           pPublicKeyTemplate, ulPublicKeyAttributeCount, &pub);
            if (rv == CKR_OK) {
                /* Copy the CKA_PARAMETER_SET attribute from the public key template
                 * to the private key one to properly initialize the private key in
                 * NewObject() below. */
                CK_ATTRIBUTE_PTR privTmplCpy = (CK_ATTRIBUTE_PTR)XMALLOC(
                ...
                if (privTmplCpy == NULL) {
                    ...
                    for (unsigned int i = 0;
                                          i < ulPrivateKeyAttributeCount; i++) {
                        privTmplCpy[i] = pPrivateKeyTemplate[i];
                    }
                    /* Find the CKA_PARAMETER_SET in the public key template */
                    for (unsigned int i = 0;
                                           i < ulPublicKeyAttributeCount; i++) {
                    ...
                    rv = NewObject(session, CKK_ML_KEM, CKO_PRIVATE_KEY,
                                   privTmplCpy, ulPrivateKeyAttributeCount + 1,
                                   &priv);
